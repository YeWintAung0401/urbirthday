...
const config = {
  birthdate: 'Nov 16, 2024',
  name: 'Wink ❤️',
  wish: 'Wishing You Joy And Love!',
};
...
x = setInterval(function() {
  ...
  let w = (c.width = window.innerWidth),
    ...
    hh = h / 2,
    opts = {
      strings: ['HAPPY', 'BIRTHDAY!', config.name, config.wish],
      charSize: 30,
      charSpacing: 35,
      lineHeight: 40,
  ...
  function playMusic() {
    let audio = new Audio("HBD.mp3");  // Ensure this points to the correct audio file
    audio.play();  // Play the audio when the giftbox is clicked
  } 

  function anim() {
    window.requestAnimationFrame(anim);

    ctx.fillStyle = '#ffc0fc';
